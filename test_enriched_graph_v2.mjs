...
async function testEnrichedMultiStage() {
    console.log(`\n--- [NEW STAGE] Testing getEnrichedGraphContext with query: "${query}" ---`);
...
(async () => {
    // I'll first update the service with better search logic
    // But for now let's just see current failure with my previous fix
    await testEnrichedMultiStage();
})();
