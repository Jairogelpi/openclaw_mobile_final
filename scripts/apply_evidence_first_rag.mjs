...
const migrationPath = path.join(__dirname, '..', 'migrations', 'evidence_first_rag_2026.sql');
...
async function main() {
    ...
    try {
        ...
        console.log('Evidence-First RAG migration applied successfully.');
    } finally {
        await sql.end();
    }
}
...
main().catch(error => {
    console.error('Failed to apply Evidence-First RAG migration:', error);
    process.exit(1);
});
