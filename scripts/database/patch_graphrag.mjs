...
async function patch() {
    try {
        await sql`DROP FUNCTION IF EXISTS public.graphrag_traverse(text, vector, int, text);`;
        await sql`
...
BEGIN
    ...
    hop1_discovered_nodes AS (
        SELECT DISTINCT unnest(ARRAY[source_node, target_node]) as node_name, resonance, seed_origin FROM hop1_edges
    ),
    hop2_edges AS (
        SELECT DISTINCT ON (e.source_node, e.relation_type, e.target_node)
    ...
    hop2_discovered_nodes AS (
        SELECT DISTINCT unnest(ARRAY[source_node, target_node]) as node_name, resonance, seed_origin FROM hop2_edges
    ),
    hop3_edges AS (
        SELECT DISTINCT ON (e.source_node, e.relation_type, e.target_node)
...
$function$;
        `;
        console.log("✅ Function graphrag_traverse patched to accept uuid.");
    } catch (e) {
