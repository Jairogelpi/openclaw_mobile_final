...
async function fetchRelationEdges(clientId, limit = 50) {
    const { data, error } = await supabase
        ...
        .select('source_node, target_node, relation_type, context, last_seen, weight, support_count, stable_score, stability_tier')
        .eq('client_id', clientId)
        .in('stability_tier', ['provisional', 'stable'])
        .not('relation_type', 'in', '("[RELACIONADO_CON]","[HABLA_DE]")')
...
async function fetchRelationMentions(clientId, limit = 80) {
    const { data, error } = await supabase
        ...
        .select('source_node, target_node, relation_type, context, last_seen, support_count, stable_score, stability_tier, metadata')
        .eq('client_id', clientId)
        .in('stability_tier', ['provisional', 'stable'])
        .not('relation_type', 'in', '("[RELACIONADO_CON]","[HABLA_DE]")')
...
function isSpecificRelationType(relationType) {
    return !['[RELACIONADO_CON]', '[HABLA_DE]'].includes(String(relationType || '').trim());
}

function hasSubstantiveRelationContext(value = '') {
...
function relationKey(sourceNode, relationType, targetNode) {
    return `${sourceNode}|${relationType}|${targetNode}`;
}

function buildConnectedRelationSet(edges = [], mentions = []) {
    return new Set(
        [...edges, ...mentions]
            .filter(item => item.source_node && item.target_node && item.relation_type)
            .map(item => relationKey(item.source_node, item.relation_type, item.target_node))
    );
}
...
function buildRelationCases(edges) {
    return edges
        .filter(edge =>
            ...
            edge.target_node &&
            edge.relation_type &&
            isSpecificRelationType(edge.relation_type) &&
            hasSubstantiveRelationContext(edge.context || '') &&
            Number(edge.support_count || 0) >= 1
        )
        .map(edge => ({
...
function buildRelationMentionCases(mentions) {
    return mentions
        .filter(item =>
            ...
            item.target_node &&
            item.relation_type &&
            isSpecificRelationType(item.relation_type) &&
            hasSubstantiveRelationContext(item.context || item.metadata?.evidence || '') &&
            (Number(item.support_count || 0) >= 2 || Number(item.stable_score || 0) >= 7)
        )
        .map(item => ({
            category: 'relationship_lookup',
...
function buildRecentMemoryCases(memories = [], limit = 16) {
    return memories
        .filter(memory => memory.created_at && memory.metadata?.contactName && memory.metadata?.remoteId)
        .slice(0, limit)
        .map(memory => ({
            category: 'recent_memory',
            style_tag: String(memory.metadata?.remoteId || '').endsWith('@g.us') ? 'group_chat' : 'recent_dense',
            query: `que ha pasado recientemente con ${memory.metadata.contactName}`,
            expected_mode: 'answer',
            expected_entities: [memory.metadata.contactName],
            expected_remote_ids: [memory.metadata.remoteId],
            expected_substrings: [memory.metadata.contactName],
            expected_evidence_kinds: ['memory_chunk'],
...
function buildRelationAbstainCases(identityRows = [], connectedRelationKeys = new Set(), limit = 14) {
    const rows = identityRows
        .filter(row => row?.canonical_name && !String(row.remote_id || '').endsWith('@g.us'))
        .slice(0, 18);
    const cases = [];
    ...
    for (let i = 0; i < rows.length; i += 1) {
        for (let j = i + 1; j < rows.length; j += 1) {
            ...
            if (left.remote_id === right.remote_id) continue;

            const alreadyConnected = [...connectedRelationKeys].some(key =>
                key.startsWith(`${left.canonical_name}|`) && key.endsWith(`|${right.canonical_name}`)
                || key.startsWith(`${right.canonical_name}|`) && key.endsWith(`|${left.canonical_name}`)
            );
            if (alreadyConnected) continue;

            cases.push({
                category: 'relationship_abstain',
                style_tag: 'relationship_negative',
                query: `que relacion hay entre ${left.canonical_name} y ${right.canonical_name}`,
                expected_mode: 'abstain',
                expected_citation_min: 0,
                expected_entities: [left.canonical_name, right.canonical_name],
                expected_remote_ids: [],
                expected_substrings: [],
                notes: {
                    negative_pair: true,
                    left_remote_id: left.remote_id,
                    right_remote_id: right.remote_id
                }
            });
...
export async function seedEvalCasesForClient({
    ...
    const connectedRelationKeys = buildConnectedRelationSet(relationEdges, relationMentions);
    ...
    const seededCases = uniqueCases([
        ...buildIdentityCases(identityRows.slice(0, 32), memoriesByRemoteId),
        ...buildRelationCases(relationEdges.slice(0, 60)),
        ...buildRelationMentionCases(relationMentions.slice(0, 90)),
        ...buildRecentMemoryCases(memories, 18),
        ...buildRelationAbstainCases(identityRows, connectedRelationKeys, 16),
        ...buildGroupCases(memories),
        ...buildAbstainCases(12)
    ]).slice(0, targetCount);
