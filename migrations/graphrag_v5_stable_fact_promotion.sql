-- ====================================================================
-- OPENCLAW GRAPHRAG V5: STABLE FACT PROMOTION
-- ====================================================================

ALTER TABLE public.knowledge_nodes
ADD COLUMN IF NOT EXISTS support_count integer NOT NULL DEFAULT 1,
ADD COLUMN IF NOT EXISTS stable_score double precision NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS stability_tier text NOT NULL DEFAULT 'candidate',
...
SET
    support_count = COALESCE(support_count, 1),
    stable_score = COALESCE(stable_score, 0),
    stability_tier = COALESCE(NULLIF(stability_tier, ''), 'candidate'),
    last_seen = COALESCE(last_seen, created_at, now());

UPDATE public.knowledge_edges
SET
    support_count = COALESCE(support_count, GREATEST(weight, 1), 1),
    stable_score = COALESCE(stable_score, 0),
    stability_tier = COALESCE(NULLIF(stability_tier, ''), 'candidate');

CREATE INDEX IF NOT EXISTS idx_knowledge_nodes_stability
    ON public.knowledge_nodes(client_id, stability_tier, stable_score DESC);

CREATE INDEX IF NOT EXISTS idx_knowledge_edges_stability
    ON public.knowledge_edges(client_id, stability_tier, stable_score DESC);
