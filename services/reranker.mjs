...
function computeRelationshipEvidenceBias(plan, candidate) {
    if (plan?.intent !== 'relationship_lookup') return 0;

    const source = normalizeComparableText(candidate.metadata?.source_node || candidate.speaker || '');
    const target = normalizeComparableText(candidate.metadata?.target_node || '');
    const evidence = normalizeComparableText(candidate.evidence_text || '');
    const supportCount = Number(candidate.metadata?.support_count || 0);

    let score = 0;
    if (source && evidence.includes(source)) score += 0.35;
    if (target && evidence.includes(target)) score += 0.35;
    if (candidate.metadata?.fact_type === 'relationship_edge') score += 0.2;
    else if (candidate.metadata?.fact_type === 'relationship_memory') score += 0.12;
    score += Math.min(supportCount, 3) * 0.08;
...
function computeIntentBias(plan, candidate) {
    // 2026 Neutral Edition: Intent is an emergent property of semantic similarity.
    // We still provide a small boost to facts as they are ground truth.
    if (candidate.source_kind === 'fact' || candidate.source_kind === 'graph_node') return 0.2;
    return 0;
}
...
export async function rerankEvidenceCandidates({
    ...
    const ranked = (candidates || []).map(candidate => {
        ...
        const relationshipEvidenceBias = computeRelationshipEvidenceBias(plan, candidate);
        ...
        return {
            ...
            relationship_evidence_bias: Number(relationshipEvidenceBias.toFixed(4)),
